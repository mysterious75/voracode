# Remove `packages/voracode/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/voracode/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the voracode legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/voracode/src/storage/db.ts` are concentrated in 22 source files:

- `packages/voracode/src/account/repo.ts`
- `packages/voracode/src/cli/cmd/db.ts`
- `packages/voracode/src/cli/cmd/import.ts`
- `packages/voracode/src/cli/cmd/stats.ts`
- `packages/voracode/src/control-plane/workspace.ts`
- `packages/voracode/src/index.ts`
- `packages/voracode/src/node.ts`
- `packages/voracode/src/permission/index.ts`
- `packages/voracode/src/project/project.ts`
- `packages/voracode/src/server/projectors.ts`
- `packages/voracode/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/voracode/src/server/shared/fence.ts`
- `packages/voracode/src/session/message-v2.ts`
- `packages/voracode/src/session/projectors.ts`
- `packages/voracode/src/session/prompt.ts`
- `packages/voracode/src/session/session.ts`
- `packages/voracode/src/session/todo.ts`
- `packages/voracode/src/share/share-next.ts`
- `packages/voracode/src/storage/db.ts`
- `packages/voracode/src/sync/index.ts`
- `packages/voracode/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy voracode database wrapper; `packages/voracode/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the voracode wrapper.
...
Status: Completed. `SyncEvent` and the voracode projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy voracode database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy voracode database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy voracode database wrapper.
...
All migration groups are complete or superseded. `packages/voracode/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No voracode data-migration group remains.

The previous voracode `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate voracode data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/voracode`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/voracode/src`
- `bun typecheck` from `packages/voracode`
- Relevant package tests from `packages/voracode`, not the repo root
