...
Both `action` and `resource` use voracode's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.voracode` files is intentionally deferred until `.voracode` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether voracode permits an operation through its managed execution paths.
