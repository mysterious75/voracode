...
The voracode server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@voracode-ai/llm` tool loop after replacing its
...
- plugins get global "voracode" instance like in that post i showed
- voracode instance has stuff like `voracode.session.prompt()` or
  `voracode.tool.register({...})`
...
- decide whether to preserve deprecated `@voracode-ai/llm` orchestration exports
