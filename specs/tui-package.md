...
Move the canonical Voracode terminal application from
`packages/voracode/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/voracode ---\
                      > @voracode-ai/tui -> @voracode-ai/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/voracode`,
`packages/cli`, or `@voracode-ai/core`.

The SDK is the TUI's Voracode boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/voracode`.
- Do not replace private `packages/voracode` imports with `@voracode-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@voracode-ai/tui`.

## Ownership Boundary

### `@voracode-ai/tui` Owns
...
- Voracode domain data displayed by the TUI
...
```text
packages/voracode/src/cli/cmd/tui
```

Its private dependency on `packages/voracode` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/voracode/src/*`.
...
Status: Completed. The private `@voracode-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@voracode-ai/tui`.
...
- The package has no dependency on `voracode`, `@voracode-ai/cli`, or
  `@voracode-ai/core`.
...
primitives now live in `@voracode-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@voracode-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Voracode globals or process state;
...
Replace process-global Voracode state with resolved TUI inputs.
...
`@voracode-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/voracode` initially.
...
canonical in `@voracode-ai/tui`. Configured references resolve through the new
...
adapters rather than Voracode domain implementations.

Make the SDK the only Voracode domain boundary used by the TUI.
...
- Domain-facing TUI code imports Voracode data and operations only from
  `@voracode-ai/sdk`.
...
`@voracode-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/voracode`.
...
- The package has no imports from `packages/voracode`, `packages/cli`, or
  `@voracode-ai/core`.
...
public `@voracode-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/voracode`.
- Keep the legacy embedded worker and server startup in `packages/voracode`.
- Change those adapters to load config, create transport inputs, and call the
  public `@voracode-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/voracode`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@voracode-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@voracode-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Voracode domain state.
...
cd packages/voracode && bun typecheck
...
rg '@voracode-ai/core|packages/voracode|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/voracode/src packages/voracode/test
```
...
- Build the current-platform `packages/voracode` binary.
