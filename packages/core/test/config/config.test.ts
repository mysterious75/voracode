...
import { ConfigProvider } from "@voracode-ai/core/config/provider"
...
import { ConfigMigrateV1 } from "@voracode-ai/core/v1/config/migrate"
...
import { Policy } from "@voracode-ai/core/policy"
...
describe("Config", () => {
  ...
  it.live("loads voracode JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "voracode.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "voracode.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "voracode.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "voracode.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "voracode.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "voracode.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "voracode.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .voracode configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".voracode"), { recursive: true })
            await fs.mkdir(path.join(directory, ".voracode"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "voracode.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "voracode.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "voracode.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "voracode.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "voracode.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".voracode", "voracode.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".voracode", "voracode.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".voracode")),
              AbsolutePath.make(path.join(directory, ".voracode")),
            ])
