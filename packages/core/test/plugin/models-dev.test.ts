...
import { ModelsDevPlugin } from "@voracode-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.VORACODE_MODELS_PATH,
          disabled: Flag.VORACODE_DISABLE_MODELS_FETCH,
        }
        Flag.VORACODE_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.VORACODE_MODELS_PATH = previous.path
          Flag.VORACODE_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
