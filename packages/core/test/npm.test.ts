...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@voracode/acme")).toBe("@voracode/acme")
    expect(Npm.sanitize("@voracode/acme@1.0.0")).toBe("@voracode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/voracode/acme.git"
    const expected = win ? "acme@git+https_//github.com/voracode/acme.git" : spec
