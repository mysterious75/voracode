...
import { InstructionContext } from "@voracode-ai/core/instruction-context"
...
describe("InstructionContext", () => {
  ...
  it.effect("honors the project instruction opt-out", () =>
    Effect.gen(function* () {
      const previous = process.env.VORACODE_DISABLE_PROJECT_CONFIG
      let scanned = false
      process.env.VORACODE_DISABLE_PROJECT_CONFIG = "1"
      ...
      yield* SystemContextRegistry.Service.pipe(
        ...
        Effect.ensuring(
          Effect.sync(() => {
            if (previous === undefined) delete process.env.VORACODE_DISABLE_PROJECT_CONFIG
            else process.env.VORACODE_DISABLE_PROJECT_CONFIG = previous
          }),
        ),
      )
