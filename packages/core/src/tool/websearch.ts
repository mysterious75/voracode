...
import { ToolFailure } from "@voracode-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@voracode/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.VORACODE_WEBSEARCH_PROVIDER === "exa" || process.env.VORACODE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.VORACODE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("VORACODE_EXPERIMENTAL") || truthy("VORACODE_ENABLE_EXA") || truthy("VORACODE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("VORACODE_ENABLE_PARALLEL") || truthy("VORACODE_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `voracode/${InstallationVersion}`,
