...
import type { IntegrationOAuthMethodRegistration } from "@voracode-ai/plugin/v2/effect/integration"
import { define } from "@voracode-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@voracode-ai/sdk/v2/types"
...
const defaultServer = "https://console.voracode.ai"
const clientID = "voracode-cli"
...
export const voracodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "voracode",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("voracodePlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("voracode")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Voracode provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("voracode", (integration) => {
        integration.name = "Voracode"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "voracode", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("voracode")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("voracode")
      ...
      const item = catalog.provider.get(ProviderV2.ID.voracode)
      if (!item) return
      const hasKey = Boolean(process.env.VORACODE_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("voracode")),
