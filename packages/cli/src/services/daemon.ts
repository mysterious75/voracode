...
import { createvoracodeClient } from "@voracode-ai/sdk/v2/client"
import { ServerAuth } from "@voracode-ai/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createvoracodeClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@voracode/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createvoracodeClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createvoracodeClient({ baseUrl: connection.url, headers: connection.headers })
    })
