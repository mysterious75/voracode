...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      VORACODE_CLI_NAME: `'${binary}'`,
      VORACODE_MODELS_DEV: modelsData,
      VORACODE_CHANNEL: `'${Script.channel}'`,
      VORACODE_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
  ...
  await Bun.write(
    ...
    JSON.stringify(
      {
        name: `@voracode-ai/${name}`,
        version: Script.version,
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/mysterious75/voracode.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
