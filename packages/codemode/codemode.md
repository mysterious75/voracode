...
This is the living design and status document for `@voracode-ai/codemode` and its existing V2 Voracode adapter.
...
`packages/voracode/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
