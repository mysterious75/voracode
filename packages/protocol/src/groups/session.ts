...
import { PromptInput } from "@voracode-ai/schema/prompt-input"
import { Session } from "@voracode-ai/schema/session"
import { Project } from "@voracode-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@voracode-ai/schema/schema"
...
import { SessionEvent } from "@voracode-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Voracode process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Voracode process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
