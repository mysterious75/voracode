...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.voracode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `voracode.settings.dat` is special: it maps to the `voracode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"voracode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // voracode.settings.dat â†’ the electron settings store ("voracode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "voracode.global.dat").
  const storeName = filename === "voracode.settings.dat" ? "voracode.settings" : filename
