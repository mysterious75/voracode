...
import { type WslCommandLine, resolveWslvoracode, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const voracode = await resolveWslvoracode(distro)
  if (!voracode) throw new Error(`Voracode is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "voracode"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export VORACODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export VORACODE_CLIENT=desktop",
    `export VORACODE_SERVER_USERNAME=${shellEscape(username)}`,
    `export VORACODE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(voracode)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
