...
let releasevoracodeResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectvoracodeVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectvoracodeVersion("1.14.35", "1.16.2")).toThrow(
    "Voracode update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating Voracode", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.voracode/bin/voracode",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, voracodeChecks: {} })
})
...
test("ignores stale background Voracode checks after removing a WSL server", async () => {
  persistedServers = []
  releasevoracodeResolve = undefined
  ...
  await waitFor(() => !!releasevoracodeResolve)
  await controller.removeServer("wsl:Debian")
  releasevoracodeResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().voracodeChecks).toEqual({})
})

test("ignores stale startup Voracode checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking Voracode", async () => {
  persistedServers = []
  const started: string[] = []
  const release = new Map<string, () => void>()
  const voracode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolvevoracode: async (distro) => {
      voracode.push(distro)
      return "/home/me/.voracode/bin/voracode"
    },
  })
  ...
  expect(voracode).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(voracode).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().voracodeChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check Voracode in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const voracode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(voracode).toEqual(["Debian"])
  expect(Object.keys(controller.getState().voracodeChecks)).toEqual(["Debian"])
})
...
function testControllerOptions() {
  return {
    ...
    resolvevoracode: async () => {
      await new Promise<void>((resolve) => {
        releasevoracodeResolve = resolve
      })
      return "/home/me/.voracode/bin/voracode"
    },
  }
