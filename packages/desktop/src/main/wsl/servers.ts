...
import { expectvoracodeVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolvevoracode?: typeof resolveWslvoracode
...
export function createWslServersController(
  ...
  const setvoracodeCheck = (distro: string, check: WslvoracodeCheck) => {
    setState({
      voracodeChecks: {
        ...state.voracodeChecks,
        [distro]: check,
      },
    })
  }

  const checkvoracode = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolvevoracode ?? resolveWslvoracode)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return voracodeCheck(distro, resolved, version, appVersion)
  }

  const refreshvoracodeCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setvoracodeCheck(distro, await checkvoracode(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const voracodeChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.voracodeChecks[distro])
        .map(async (distro) => [distro, await checkvoracode(distro, opts)] as const),
    )
    if (voracodeChecks.length) {
      setState({ voracodeChecks: { ...state.voracodeChecks, ...Object.fromEntries(voracodeChecks) } })
    }
  }
  ...
  const refreshvoracodeCheckBackground = (id: string, distro: string) => {
    void checkvoracode(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setvoracodeCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl voracode check failed", { id, distro, message })
      })
  }

  const refreshvoracodeChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkvoracode(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setvoracodeCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl voracode check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshvoracodeCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installvoracode(name: string) {
      await runJob({ kind: "install-voracode", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslvoracode(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "Voracode installation failed")
        }
        await refreshvoracodeCheck(name, { signal: abort.signal })
        expectvoracodeVersion(state.voracodeChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.voracodeChecks, distro) : {}),
      })
    },
...
function voracodeCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "voracode is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "voracode is installed but could not run",
    }
  }
  return {
    distro,
