...
import {
  ...
  useCommand,
  useWslServers,
} from "@voracode-ai/app"
import type { UpdaterState } from "@voracode-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@voracode-ai/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.VORACODE_CHANNEL === "prod" &&
...
const deepLinkEvent = "voracode:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__VORACODE__ ??= {}
  const pending = window.__VORACODE__.deepLinks ?? []
  window.__VORACODE__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `voracode.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://voracode.ai/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("voracode.global.dat").getItem("language")
