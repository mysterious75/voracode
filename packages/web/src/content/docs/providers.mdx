---
title: Providers
description: Using any LLM provider in Voracode.
---

import config from "../../../config.mjs"
export const console = config.console

Voracode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Voracode config.
...
You can customize the providers through the `provider` section in your Voracode
config.
...
Both options take an array of model IDs â€” the same IDs shown in the `/models` picker.
...
Voracode Zen is a list of models provided by the Voracode team that have been
tested and verified to work well with Voracode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Voracode Zen.
:::

1. Run the `/connect` command in the TUI, select `Voracode Zen`, and head to [voracode.ai/auth](https://voracode.ai/zen).
...
It works like any other provider in Voracode and is completely optional to use.

---

## Voracode Go

Voracode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Voracode team that have been
tested and verified to work well with Voracode.

1. Run the `/connect` command in the TUI, select `Voracode Go`, and head to [voracode.ai/auth](https://voracode.ai/zen).
...
To use Amazon Bedrock with Voracode:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running voracode:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `voracode.json`:
...
Voracode. Anthropic explicitly prohibits this.

Previous versions of Voracode came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Voracode with zero setup:
...
You can configure voracode to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for voracode to work properly.
   :::
...
3. Enter your **Account ID** when prompted.
   ...
   ```txt
   â”Œ Enter your Cloudflare Account ID
   â”‚
   â”‚
   â”” enter
   ```
...
4. Enter your **Gateway ID** when prompted.
   ...
   ```txt
   â”Œ Enter your Cloudflare AI Gateway ID
   â”‚
   â”‚
   â”” enter
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your voracode config.
...
Voracode supports two authentication methods:

- **OAuth (Recommended)** â€” Sign in to your DigitalOcean account; Voracode uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Voracode. Sign in and approve.

   :::note
   Voracode requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference â€” no separate Model Access Key is created.
   :::
...
2. Run the `/connect` command and select **DigitalOcean**, then **Paste Model Access Key**.
   ...
   ```txt
   â”Œ Enter your DigitalOcean Model Access Key
   â”‚
   â”‚
   â”” enter
   ```
...
Inference Routers let you define a routing policy across multiple models â€” picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Voracode surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model â€” Voracode forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Voracode is experimental. Features, configuration, and
behavior may change in future releases.
:::

Voracode integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
Voracode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Voracode
to only use your own GitLab-hosted instance, add the following to your
`voracode.json` file. It is also recommended to disable session sharing.
...
More documentation on [voracode-gitlab-auth](https://www.npmjs.com/package/voracode-gitlab-auth) homepage.
...
When a `duo-workflow-*` model is selected, Voracode will:
...
4. Route tool execution requests through Voracode's permission-gated tool system
...
To use your GitHub Copilot subscription with voracode:
...
To use GMI Cloud with Voracode:
...
To use Google Vertex AI with Voracode:
...
2. Set the required environment variables:
   ...
   Set them while running voracode.
...
In the event you see a feature or model from Helicone that isn't configured automatically through voracode, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [voracode-helicone-session](https://github.com/H2Shami/voracode-helicone-session) plugin to automatically log each Voracode conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Voracode conversation listed as a separate session.
...
You can configure voracode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure voracode to use local models through LM Studio.
...
You can configure voracode to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Voracode. See the [Ollama integration docs](https://docs.ollama.com/integrations/voracode) for details.
:::
...
To use Ollama Cloud with Voracode:
...
3. Copy the API key for use in Voracode.
...
6. **Important**: Before using cloud models in Voracode, you must pull the model information locally:
...
Voracode Zen is a list of tested and verified models provided by the Voracode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Voracode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Voracode Zen**.

   ```txt
   /connect
   ```

3. Enter your Voracode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your voracode config.
...
5. You can also customize them through your voracode config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Voracode:
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=voracode&utm_medium=docs&utm_campaign=cortex-provider).

voracode's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.

Voracode supports two authentication methods:

- **Browser OAuth (Recommended)** â€” sign in with your IdP/SSO; no secrets to manage, tokens refresh automatically.
- **Manual bearer token** â€” paste a PAT or JWT from the Snowflake console.
...
2. Select **Login with Snowflake (External Browser)**.
   ...
   â”Œ Select auth method
   â”‚
   â”‚ Login with Snowflake (External Browser)
   â”‚ Paste PAT or bearer token manually
   â””
   ```
...
3. Enter your [account identifier](https://docs.snowflake.com/en/user-guide/admin-account-identifier) when prompted, for example `myorg-myaccount` or `xy12345.us-east-1`.
   ...
   ```txt
   â”Œ Snowflake Account Identifier
   â”‚
   â”‚
   â”” enter
   ```
...
5. Complete sign-in in the browser that opens. Voracode captures the OAuth callback automatically and stores the token â€” no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting voracode:
...
:::note
`SNOWFLAKE_CORTEX_TOKEN` accepts a PAT or JWT only â€” the browser OAuth flow is available via `/connect` only and cannot be configured through an environment variable. `SNOWFLAKE_CORTEX_PAT` is still supported for backward compatibility.
:::

The model catalog is provided automatically. A minimal `voracode.json` is all that's needed:
...
You can also customize models through your voracode config. Here's an example of specifying provider routing order.
...
#### Option A â€” SuperGrok OAuth (browser login)
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. Voracode opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
Voracode refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B â€” SuperGrok device-code (headless / remote server / VPS)

Use this when Voracode is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running Voracode â€” instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, â€¦).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. Voracode prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. Voracode polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
:::tip
You can use any OpenAI-compatible provider with voracode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `voracode.json` file in your project directory:
...
The `limit` fields allow Voracode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `voracode auth list` to see if the credentials
...
2. For custom providers, check the voracode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your voracode config.
