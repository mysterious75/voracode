...
Voracode Go is a low cost subscription â€” **$5 for your first month**, then **$10/month** â€” that gives you reliable access to popular open coding models.

Go works like any other provider in Voracode. You subscribe to Voracode Go and
...
Voracode Go gives you access to these models for **$5 for your first month**, then **$10/month**.

---

## How it works

Voracode Go works like any other provider in Voracode.

1. You sign in to **<a href={console}>Voracode Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `Voracode Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to Voracode Go.
:::
...
Voracode Go includes the following limits:
...
- GLM-5.2/5.1 â€” 700 input, 52,000 cached, 150 output tokens per request
- Kimi K2.7 Code/K2.6 â€” 870 input, 55,000 cached, 200 output tokens per request
...
uses the format `voracode-go/<model-id>`. For example, for Kimi K2.7 Code, you would
use `voracode-go/kimi-k2.7-code` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with Voracode as well.
