---
title: Policies
description: Control which configured resources Voracode may use.
---

Policies control whether Voracode may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `voracode.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether Voracode may use a resource such as an LLM provider.
...
Voracode currently supports one policy action:
