---
title: Config
description: Using the Voracode JSON config.
---

You can configure Voracode using a JSON config file.

---

## Format

Voracode supports both **JSON** and **JSONC** (JSON with Comments) formats.
...
1. **Remote config** (from `.well-known/voracode`) - organizational defaults
2. **Global config** (`~/.config/voracode/voracode.json`) - user preferences
3. **Custom config** (`VORACODE_CONFIG` env var) - custom overrides
4. **Project config** (`voracode.json` in project) - project-specific settings
5. **`.voracode` directories** - agents, commands, plugins
6. **Inline config** (`VORACODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/voracode/` on macOS) - admin-controlled
...
:::note
The `.voracode` and `~/.config/voracode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/voracode` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
Place your global Voracode config in `~/.config/voracode/voracode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/voracode/tui.json`.
...
Add `voracode.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Voracode starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `VORACODE_CONFIG` environment variable.
...
Specify a custom config directory using the `VORACODE_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.voracode` directory, and should
...
The custom directory is loaded after the global config and `.voracode` directories, so it **can override** their settings.
...
Drop an `voracode.json` or `voracode.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/voracode/` |
| Linux    | `/etc/voracode/`                         |
| Windows  | `%ProgramData%\voracode`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Voracode reads managed preferences from the `ai.voracode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Voracode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.voracode.managed.plist`
2. `/Library/Managed Preferences/ai.voracode.managed.plist`

The plist keys map directly to `voracode.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.voracode.managed` PayloadType. The Voracode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.voracode.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.voracode.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.voracode</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`voracode.ai/config.json`**](https://voracode.ai/config.json).

TUI config uses [**`voracode.ai/tui.json`**](https://voracode.ai/tui.json).
...
Use `VORACODE_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `voracode.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `voracode serve` and `voracode web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Voracode server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `voracode.local`. Useful for running multiple instances on the same network.
...
If not specified, Voracode will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Voracode config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Voracode tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Voracode actions on configured resources. Currently, policies can control which providers Voracode may use.
...
Voracode normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Voracode omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/voracode/agents/` or `.voracode/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Voracode will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`voracode run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/voracode/commands/` or `.voracode/commands/`. [Learn more here](/docs/commands).
...
Voracode uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Voracode will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, voracode **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Voracode with custom tools, hooks, and integrations.

Place plugin files in `.voracode/plugins/` or `~/.config/voracode/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict Voracode to only use specific providers rather than disabling them one by one.
