...
Create `.voracode/commands/test.md`:
...
You can add custom commands through the Voracode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Voracode [config](/docs/config):
...
voracode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
