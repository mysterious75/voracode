---
title: TUI
description: Using the Voracode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Voracode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Voracode starts the TUI for the current directory.
...
When using the Voracode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Voracode. Allows you to select from available providers and add their API keys.
...
Exit Voracode. _Aliases_: `/quit`, `/q`
...
This is separate from `voracode.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Voracode waits after the leader key. Defaults to `2000`.
...
Use `VORACODE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `voracode.default`.
