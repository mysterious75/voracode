...
Puoi aggiungere strumenti esterni a Voracode usando il _Model Context Protocol_, o MCP. Voracode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Voracode Config](https://voracode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/voracode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Voracode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Voracode:
...
Se il server richiede autenticazione, Voracode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `voracode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Voracode memorizzera' i token in modo sicuro in `~/.local/share/voracode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Voracode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Voracode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Voracode al tuo account Sentry.
