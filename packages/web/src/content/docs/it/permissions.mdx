...
Voracode usa la configurazione `permission` per decidere se una determinata azione deve essere eseguita automaticamente, se deve chiederti conferma o se deve essere bloccata.
...
- `"allow"` â€” esegui senza approvazione
- `"ask"` â€” chiedi approvazione
- `"deny"` â€” blocca l'azione
...
Usa `external_directory` per consentire chiamate a strumenti che toccano percorsi al di fuori della directory di lavoro da cui e' stato avviato Voracode. Si applica a qualsiasi strumento che accetta un path come input (ad esempio `read`, `edit`, `glob`, `grep` e molti comandi `bash`).
...
I permessi di Voracode sono indicizzati per nome dello strumento, piu' un paio di guardrail di sicurezza:

- `read` â€” lettura di un file (corrisponde al percorso del file)
- `edit` â€” tutte le modifiche ai file (include `edit`, `write`, `patch`)
- `glob` â€” ricerca file tramite glob (corrisponde al pattern glob)
- `grep` â€” ricerca nel contenuto (corrisponde al pattern regex)
- `bash` â€” esecuzione comandi di shell (corrisponde a comandi parsati come `git status --porcelain`)
- `task` â€” avvio subagenti (corrisponde al tipo di subagente)
- `skill` â€” caricamento di una skill (corrisponde al nome della skill)
- `lsp` â€” esecuzione query LSP (attualmente non granulare)
- `webfetch` â€” fetch di un URL (corrisponde all'URL)
- `websearch` â€” ricerca web (corrisponde alla query)
- `external_directory` â€” si attiva quando uno strumento tocca percorsi fuori dalla working directory del progetto
- `doom_loop` â€” si attiva quando la stessa chiamata a uno strumento si ripete 3 volte con input identico

---

## Default

Se non specifichi nulla, Voracode parte da default permissivi:
...
Quando Voracode chiede approvazione, la UI offre tre esiti:

- `once` â€” approva solo questa richiesta
- `always` â€” approva richieste future che corrispondono ai pattern suggeriti (per il resto della sessione corrente di Voracode)
- `reject` â€” nega la richiesta
