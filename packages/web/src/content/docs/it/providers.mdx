---
title: Provider
description: Usa qualsiasi provider LLM in Voracode.
---

import config from "../../../../config.mjs"
export const console = config.console

Voracode usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Voracode.
...
Voracode Zen e una lista di modelli forniti dal team Voracode, testati e
verificati per funzionare bene con Voracode. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Voracode Zen.
:::

1. Esegui `/connect` nella TUI, seleziona voracode e vai su [voracode.ai/auth](https://voracode.ai/auth).
...
Funziona come qualsiasi altro provider in Voracode ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Voracode:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui voracode:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `voracode.json`:
   ...
   ***
   ...
   :::note
   L'opzione `endpoint` Ã¨ un alias per l'opzione generica `baseURL`, usando terminologia specifica AWS. Se vengono specificati sia `endpoint` sia `baseURL`, `endpoint` ha la precedenza.
   :::
   ...
   ***
   ...
   Amazon Bedrock usa la seguente prioritÃ  di autenticazione:
   ...
   :::note
   Quando Ã¨ impostato un bearer token (tramite `/connect` o `AWS_BEARER_TOKEN_BEDROCK`), ha la precedenza su tutti i metodi di credenziali AWS inclusi i profili configurati.
   :::
...
:::note
Per profili di inferenza personalizzati, usa il nome del modello e del provider nella chiave e imposta la proprietÃ  `id` all'arn. Questo assicura una cache corretta.
:::
...
2. Qui puoi selezionare l'opzione **Claude Pro/Max**: aprirÃ  il tuo browser
   e ti chiederÃ  di autenticarti.
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Voracode non Ã¨ ufficialmente supportato da [Anthropic](https://anthropic.com).
:::

##### Usare chiavi API

Puoi anche selezionare **Create an API Key** se non hai un abbonamento Pro/Max. AprirÃ  il browser, ti chiederÃ  di accedere ad Anthropic e ti darÃ  un codice da incollare nel terminale.

Oppure se hai giÃ  una chiave API, puoi selezionare **Manually enter API Key** e incollarla nel terminale.

---

### Atomic Chat

Puoi configurare voracode per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
- `atomic-chat` Ã¨ l'ID personalizzato del provider. PuÃ² essere qualsiasi stringa.
...
- `name` Ã¨ il nome visualizzato del provider nell'interfaccia.
- `options.baseURL` Ã¨ l'endpoint del server locale. Modifica host e porta in base alla tua configurazione Atomic Chat.
- `models` Ã¨ una mappa di ID modello ai rispettivi nomi visualizzati. Ogni ID deve corrispondere all'`id` restituito da `GET /v1/models` â€” esegui `curl http://127.0.0.1:1337/v1/models` per elencare gli ID attualmente caricati in Atomic Chat.
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinchÃ© voracode funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di voracode.
...
GitLab Duo fornisce una chat agentic basata su AI con capacitÃ  di chiamata strumenti nativa tramite il proxy Anthropic di GitLab.
...
2. Scegli il tuo metodo di autenticazione:
   ...
   Seleziona **OAuth** e il tuo browser si aprirÃ  per l'autorizzazione.
...
3. Esegui il comando `/models` per vedere i modelli disponibili.
   ...
   - **duo-chat-opus-4-5** - PiÃ¹ capace per analisi complesse

:::note
Puoi anche specificare la variabile d'ambiente 'GITLAB_TOKEN' se non vuoi
salvare il token nello storage di auth di voracode.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
Voracode usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
Ãˆ configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Voracode
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `voracode.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
una nuova applicazione (Settings â†’ Applications) con l'
...
Maggior documentazione sulla homepage di [voracode-gitlab-auth](https://www.npmjs.com/package/voracode-gitlab-auth).

##### Configurazione

Personalizza tramite `voracode.json`:
...
Per usare il tuo abbonamento GitHub Copilot con voracode:
...
Per usare Google Vertex AI con Voracode:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui voracode.
...
:::tip
La regione `global` migliora la disponibilitÃ  e riduce gli errori senza costi extra. Usa endpoint regionali (ad es. `us-central1`) per requisiti di residenza dei dati. [Scopri di piÃ¹](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models#regional_and_global_endpoints)
:::
...
[Helicone](https://helicone.ai) Ã¨ una piattaforma di osservabilitÃ  LLM che fornisce logging, monitoraggio e analisi per le tue applicazioni AI. L'AI Gateway di Helicone instrada automaticamente le tue richieste al provider appropriato in base al modello.
...
Per altri provider e funzionalitÃ  avanzate come caching e rate limiting, controlla la [documentazione di Helicone](https://docs.helicone.ai).

#### Config opzionali

Nel caso vedessi una funzionalitÃ  o un modello di Helicone che non viene configurato automaticamente da voracode, puoi sempre configurarlo tu stesso.

Ecco la [Model Directory di Helicone](https://helicone.ai/models), ti servirÃ  per recuperare gli ID dei modelli che vuoi aggiungere.
...
Helicone supporta header personalizzati per funzionalitÃ  come caching, user tracking e gestione sessioni. Aggiungili alla configurazione del provider usando `options.headers`:
...
La funzionalitÃ  [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [voracode-helicone-session](https://github.com/H2Shami/voracode-helicone-session) per loggare automaticamente ogni conversazione di Voracode come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Voracode elencata come sessione separata.
...
| `Helicone-Property-[Name]` | Aggiungi proprietÃ  personalizzate (ad es. `Helicone-Property-Environment`) |
...
Puoi configurare voracode per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
- `llama.cpp` Ã¨ l'ID provider personalizzato. PuÃ² essere qualsiasi stringa tu voglia.
- `npm` specifica il pacchetto da usare per questo provider. Qui, `@ai-sdk/openai-compatible` Ã¨ usato per qualsiasi API OpenAI-compatible.
- `name` Ã¨ il nome visualizzato per il provider nella UI.
- `options.baseURL` Ã¨ l'endpoint per il server locale.
- `models` Ã¨ una mappa di ID modello e relative configurazioni. Il nome del modello verrÃ  visualizzato nella lista di selezione modelli.
...
Puoi configurare voracode per usare modelli locali tramite LM Studio.
...
- `lmstudio` Ã¨ l'ID provider personalizzato. PuÃ² essere qualsiasi stringa tu voglia.
- `npm` specifica il pacchetto da usare per questo provider. Qui, `@ai-sdk/openai-compatible` Ã¨ usato per qualsiasi API OpenAI-compatible.
- `name` Ã¨ il nome visualizzato per il provider nella UI.
- `options.baseURL` Ã¨ l'endpoint per il server locale.
- `models` Ã¨ una mappa di ID modello e relative configurazioni. Il nome del modello verrÃ  visualizzato nella lista di selezione modelli.
...
Puoi configurare voracode per usare modelli locali tramite Ollama.

:::tip
Ollama puÃ² auto-configurarsi per Voracode. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/voracode) per dettagli.
:::
...
- `ollama` Ã¨ l'ID provider personalizzato. PuÃ² essere qualsiasi stringa tu voglia.
- `npm` specifica il pacchetto da usare per questo provider. Qui, `@ai-sdk/openai-compatible` Ã¨ usato per qualsiasi API OpenAI-compatible.
- `name` Ã¨ il nome visualizzato per il provider nella UI.
- `options.baseURL` Ã¨ l'endpoint per il server locale.
- `models` Ã¨ una mappa di ID modello e relative configurazioni. Il nome del modello verrÃ  visualizzato nella lista di selezione modelli.
...
Per usare Ollama Cloud con Voracode:
...
3. Copia la chiave API da usare in Voracode.
...
6. **Importante**: Prima di usare modelli cloud in Voracode, devi scaricare le informazioni del modello localmente:
...
2. Qui puoi selezionare l'opzione **ChatGPT Plus/Pro**: aprirÃ  il tuo browser
   e ti chiederÃ  di autenticarti.
...
Se hai giÃ  una chiave API, puoi selezionare **Manually enter API Key** e incollarla nel terminale.

---

### Voracode Zen

Voracode Zen Ã¨ una lista di modelli testati e verificati forniti dal team Voracode. [Scopri di piÃ¹](/docs/zen).

1. Accedi a **<a href={console}>Voracode Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Voracode Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Voracode.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di voracode.
...
5. Puoi anche personalizzarli tramite la configurazione di voracode. Ecco un esempio di specifica di un provider
...
1. Vai al tuo [SAP BTP Cockpit](https://account.hana.ondemand.com/), naviga nella tua istanza di servizio SAP AI Core e crea una service key.
   ...
   :::tip
   La service key Ã¨ un oggetto JSON contenente `clientid`, `clientsecret`, `url` e `serviceurls.AI_API_URL`. Puoi trovare la tua istanza AI Core sotto **Services** > **Instances and Subscriptions** nel BTP Cockpit.
   :::
...
STACKIT AI Model Serving fornisce un ambiente di hosting completamente gestito e sovrano per modelli AI, concentrandosi su LLM come Llama, Mistral e Qwen, con la massima sovranitÃ  dei dati su infrastruttura europea.
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con Voracode:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di voracode. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con voracode. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `voracode.json` nella directory del progetto:
...
- **apiKey**: Imposta usando la sintassi variabile `env`, [scopri di piÃ¹](/docs/config#env-vars).
- **headers**: Header personalizzati inviati con ogni richiesta.
- **limit.context**: Massimi token di input accettati dal modello.
- **limit.output**: Massimi token che il modello puÃ² generare.

I campi `limit` permettono a Voracode di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `voracode auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di voracode e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config voracode.
