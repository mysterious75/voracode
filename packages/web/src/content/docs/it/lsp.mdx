---
title: Server LSP
description: Voracode si integra con i tuoi server LSP.
---

Voracode puÃ² integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

Voracode include diversi server LSP integrati per linguaggi popolari:
...
LSP Ã¨ disabilitato per impostazione predefinita. Quando Ã¨ abilitato, i server si avviano quando viene rilevata una delle estensioni qui sopra e i requisiti sono soddisfatti.

:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `VORACODE_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP Ã¨ abilitato e voracode apre un file:
...
LSP puÃ² aiutare l'agente a trovare e correggere problemi fornendo diagnostica dai server di linguaggio. Questo Ã¨ utile in alcuni progetti, ma non Ã¨ sempre un vantaggio netto.

I server di linguaggio possono andare fuori sincronizzazione, usare molta memoria, variare in base alla versione o al progetto e rallentare i workflow dell'agente. In molti progetti Ã¨ meglio far eseguire direttamente all'agente lint, typecheck o altri strumenti CLI diagnostici, cosÃ¬ gli errori rientrano nel ciclo dell'agente senza questi compromessi. Documenta questi comandi nei file di istruzioni come `AGENTS.md` o nelle skills, cosÃ¬ l'agente sa cosa eseguire. Abilita LSP quando il tuo progetto trae beneficio da feedback aggiuntivo del server di linguaggio.

---

## Configura

Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di voracode.
...
Se `lsp` Ã¨ omesso, tutti i server LSP sono disabilitati. Per disabilitare tutti i server LSP dopo che un'altra configurazione li ha abilitati, imposta `lsp` a `false`:
