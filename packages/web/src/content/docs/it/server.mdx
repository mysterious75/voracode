---
title: Server
description: Interagisci con il server voracode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `voracode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client voracode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `voracode.local` |
...
Imposta `VORACODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `voracode`, oppure imposta `VORACODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `voracode serve` sia per `voracode web`.
...
Quando esegui `voracode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server voracode per interagire con voracode in modo programmatico.
:::

Questa architettura permette a voracode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `voracode serve` per avviare un server standalone. Se la TUI di voracode e gia in esecuzione, `voracode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Voracode.
...
Il server voracode espone le seguenti API.
...
- `query` (required) â€” stringa di ricerca (fuzzy match)
- `type` (optional) â€” limita i risultati a `"file"` o `"directory"`
- `directory` (optional) â€” sovrascrive la root del progetto per la ricerca
- `limit` (optional) â€” massimo risultati (1â€“200)
- `dirs` (optional) â€” flag legacy (`"false"` restituisce solo file)
