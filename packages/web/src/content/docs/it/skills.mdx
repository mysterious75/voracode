...
Le skill degli agenti permettono a Voracode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Voracode cerca in queste posizioni:

- Config di progetto: `.voracode/skills/<name>/SKILL.md`
- Config globale: `~/.config/voracode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Voracode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.voracode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/voracode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.voracode/skills/git-release/SKILL.md` cosi':
...
Voracode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `voracode.json`:
...
**Per agenti integrati** (in `voracode.json`):
