...
Per diagnosticare problemi con Voracode, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\voracode\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `voracode --log-level DEBUG`.

---

## Archiviazione

voracode salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/voracode/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\voracode`
...
Voracode Desktop esegue in background un server locale di Voracode (il sidecar `voracode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Voracode` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/voracode/voracode.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\voracode\voracode.jsonc`
...
Voracode puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/voracode/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\voracode\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Voracode possa ricostruirla.

1. Chiudi completamente Voracode Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/voracode`
- **Linux**: elimina `~/.cache/voracode` (oppure esegui `rm -rf ~/.cache/voracode`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\voracode`

3. Riavvia Voracode Desktop.

---

### Risolvi problemi di connessione al server

Voracode Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `voracode.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `VORACODE_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `VORACODE_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, Voracode Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Voracode.

---

### Notifiche non visualizzate

Voracode Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Voracode nelle impostazioni del sistema operativo, e
...
1. Chiudi Voracode Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Voracode Desktop):

- `voracode.settings.dat` (desktop default server URL)
- `voracode.global.dat` e `voracode.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con Voracode:
...
Per capire a quali modelli hai accesso, esegui `voracode models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\voracode`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. voracode installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\voracode`

2. Riavvia voracode per reinstallare i pacchetti provider piu recenti

Questo forzera voracode a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
voracode rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
