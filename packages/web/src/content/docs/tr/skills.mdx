...
Ajan becerileri, voracode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
voracode su konumlari tarar:

- Proje konfigurasyonu: `.voracode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/voracode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda voracode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.voracode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/voracode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.voracode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
voracode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`voracode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`voracode.json` icinde):
