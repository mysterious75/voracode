...
U Voracode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Voracode koristi nasu `voracode` temu.
...
Voracode dolazi sa vise ugradenih tema.
...
- Zele da Voracode odgovara izgledu njihovog terminala
...
Temu mozete izabrati preko selektora tema komandom `/theme`. Ili je moÅ¾ete navesti u `tui.json`.
...
Voracode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/voracode/themes/*.json` ili `$XDG_CONFIG_HOME/voracode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.voracode/themes/*.json`
4. **Trenutni radni direktorij** - `./.voracode/themes/*.json`
