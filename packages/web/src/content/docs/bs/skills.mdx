---
title: VjeÅ¡tine agenata
description: "Definisite ponasanje koje se moze ponovo koristiti"
---

Agent skills omogucavaju Voracode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Voracode pretrazuje ove lokacije:

- Konfiguracija projekta: `.voracode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/voracode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Voracode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.voracode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/voracode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.voracode/skills/git-release/SKILL.md` ovako:
...
Voracode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `voracode.json`:
...
**Za ugradene agente** (u `voracode.json`):
...
## RjeÅ¡avanje problema s uÄitavanjem
