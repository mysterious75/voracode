...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Voracode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Voracode.
...
:::note
Ovaj alat je dostupan samo kada je `VORACODE_EXPERIMENTAL_LSP_TOOL=true` (ili `VORACODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Voracode provajdera ili kada je varijabla `VORACODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Voracode:
