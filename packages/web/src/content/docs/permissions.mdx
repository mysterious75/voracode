...
Voracode uses the `permission` config to decide whether a given action should run automatically, prompt you, or be blocked.
...
Start Voracode with `--auto` to automatically approve permission requests that are not explicitly denied.

```bash
voracode --auto
```

You can also use auto mode with [`voracode run`](/docs/cli#run).

```bash
voracode run --auto "Refactor this module"
```
...
Use `external_directory` to allow tool calls that touch paths outside the working directory where Voracode was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `glob`, `grep`, and many `bash` commands).
...
Voracode permissions are keyed by tool name, plus a couple of safety guards:
...
If you donâ€™t specify anything, Voracode starts from permissive defaults:
...
When Voracode prompts for approval, the UI offers three outcomes:

- `once` â€” approve just this request
- `always` â€” approve future requests matching the suggested patterns (for the rest of the current Voracode session)
