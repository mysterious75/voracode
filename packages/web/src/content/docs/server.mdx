---
title: Server
description: Interact with voracode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `voracode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an voracode client can use.
...
Set `VORACODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `voracode`, or set `VORACODE_SERVER_USERNAME` to override it. This applies to both `voracode serve` and `voracode web`.
...
When you run `voracode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the voracode server to interact with voracode programmatically.
:::

This architecture lets voracode support multiple clients and allows you to interact with voracode programmatically.

You can run `voracode serve` to start a standalone server. If you have the
voracode TUI running, `voracode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Voracode [IDE](/docs/ide) plugins.
...
The voracode server exposes the following APIs.
