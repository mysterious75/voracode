...
Custom tools are functions you create that the LLM can call during conversations. They work alongside voracode's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.voracode/tools/` directory of your project.
- Or globally, by placing them in `~/.config/voracode/tools/`.
