...
You can add external tools to Voracode using the _Model Context Protocol_, or MCP. Voracode supports both local and remote servers.
...
You can define MCP servers in your [Voracode Config](https://voracode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/voracode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Voracode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Voracode will:
...
If the server requires authentication, Voracode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `voracode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Voracode will store the tokens securely in `~/.local/share/voracode/mcp-auth.json`.
...
Your MCPs are available as tools in Voracode, alongside built-in tools. So you can manage them through the Voracode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Voracode to your Sentry account.
