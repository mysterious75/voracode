---
title: TUI
description: Arbeite mit Voracode in der Terminal-Oberflaeche.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Voracode bietet eine interaktive Terminaloberflaeche (TUI), um mit einem LLM an deinen Projekten zu arbeiten.

Ohne weitere Argumente startet Voracode die TUI im aktuellen Verzeichnis.
...
In der Voracode-TUI kannst du `/` plus Befehlsnamen eingeben, um Aktionen schnell auszufuehren.
Zum Beispiel:
...
Fuegt einen Provider zu Voracode hinzu. Erlaubt die Auswahl aus verfuegbaren Providern und das Hinzufuegen ihrer API-Keys.
...
Beendet Voracode. _Aliase_: `/quit`, `/q`
...
Dies ist getrennt von `voracode.json`, welche das Server-/Runtime-Verhalten konfiguriert.

`keybinds` wird mit den integrierten Standardwerten zusammengefÃ¼hrt, daher musst du nur die Tastenkombinationen konfigurieren, die du Ã¤ndern mÃ¶chtest.

### Optionen

- `theme` - Setzt dein UI-Theme. [Mehr dazu](/docs/themes).
- `keybinds` - Passt Tastenkombinationen an. [Mehr dazu](/docs/keybinds).
- `leader_timeout` - Steuert, wie lange Voracode nach der leader key wartet. Standard ist `2000`.
...
- `attention` - Konfiguriert Desktop-Benachrichtigungen und Sounds des TUI. StandardmÃ¤ÃŸig deaktiviert.

Verwende `VORACODE_TUI_CONFIG`, um einen benutzerdefinierten TUI-Konfigurationspfad zu laden.

### Attention

Attention lÃ¤sst das TUI dich benachrichtigen, wenn Voracode auf eine Antwort wartet, eine Berechtigung bestÃ¤tigt werden muss, ein Sitzungsfehler aufgetreten ist oder eine Sitzung abgeschlossen wurde. Aktiviere es mit `attention.enabled`; integrierte Ereignisse spielen beim Auftreten einen Sound ab. Desktop-Benachrichtigungen werden nur gesendet, wenn das Terminalfenster nicht fokussiert ist, und nicht fÃ¼r subagent-Ereignisse verwendet.

- `enabled` - Aktiviert alle Benachrichtigungen und Sounds von Attention. Standard ist `false`.
- `notifications` - Erlaubt dem TUI bei aktiviertem Attention, Desktop-Benachrichtigungen Ã¼ber das Terminal zu senden. Standard ist `true`.
- `sound` - Erlaubt bei aktiviertem Attention das Abspielen von HinweistÃ¶nen. Standard ist `true`.
- `volume` - StandardlautstÃ¤rke der HinweistÃ¶ne von `0` bis `1`. Standard ist `0.4`.
- `sound_pack` - Zu verwendende sound pack ID. Standard ist `voracode.default`.
- `sounds` - Legt eigene Sounddateien fÃ¼r `default`, `question`, `permission`, `error`, `done` oder `subagent_done` fest. Pfade kÃ¶nnen absolut, `file://` URLs oder relativ zu `tui.json` sein.
