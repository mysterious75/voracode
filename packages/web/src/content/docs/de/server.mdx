---
title: Server
description: Nutze den voracode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `voracode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein voracode-Client nutzen kann.
...
| `--mdns-domain` | Custom domain name for mDNS service | `voracode.local` |
...
Setze `VORACODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `voracode`, kann aber mit `VORACODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `voracode serve` und `voracode web`.
...
Wenn du `voracode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den voracode-Server, um voracode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `voracode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `voracode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Voracode-[IDE](/docs/ide)-Plugins.
...
Der voracode-Server stellt folgende APIs bereit.
...
- `query` (erforderlich) â€” Suchbegriff (Fuzzy-Suche)
- `type` (optional) â€” Ergebnisse auf `"file"` oder `"directory"` beschrÃ¤nken
- `directory` (optional) â€” Projektstammverzeichnis fÃ¼r die Suche Ã¼berschreiben
- `limit` (optional) â€” Maximale Ergebnisse (1â€“200)
- `dirs` (optional) â€” Legacy-Flag (`"false"` gibt nur Dateien zurÃ¼ck)
