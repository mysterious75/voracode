---
title: Regeln
description: Lege eigene Anweisungen fuer voracode fest.
---

Du kannst voracode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in voracode den Befehl `/init` aus.
...
So kann voracode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht voracode sie zu erweitern.
...
voracode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/voracode/AGENTS.md` speichern.
Diese gelten in allen voracode-Sitzungen.

Da diese Datei nicht in Git landet und nicht mit dem Team geteilt wird, eignet sie sich fuer persoenliche Regeln.

### Claude Code-KompatibilitÃ¤t

Wenn du von Claude Code kommst, unterstuetzt Voracode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/voracode/AGENTS.md` existiert)
...
Beim Start sucht voracode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/voracode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `voracode.json` oder in `~/.config/voracode/voracode.json` angeben.
...
voracode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit voracode.json

Empfohlen ist das `instructions`-Feld in der `voracode.json`:
...
Du kannst voracode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `voracode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
