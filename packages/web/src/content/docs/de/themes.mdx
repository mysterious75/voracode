...
Mit Voracode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Voracode das `voracode`-Theme.
...
Voracode liefert mehrere eingebaute Themes mit.
...
- Voracode optisch ans Terminal anpassen wollen
...
Voracode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/voracode/themes/*.json` oder `$XDG_CONFIG_HOME/voracode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.voracode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.voracode/themes/*.json`
