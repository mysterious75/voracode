...
Agent Skills erlauben Voracode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Voracode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Voracode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.voracode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/voracode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.voracode/skills/git-release/SKILL.md` zum Beispiel so:
...
Voracode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `voracode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `voracode.json`):
