...
Wenn Voracode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\voracode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `voracode --log-level DEBUG`.

---

## Speicher

voracode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/voracode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\voracode`
...
Voracode Desktop startet im Hintergrund einen lokalen Voracode-Server (`voracode-cli`-Sidecar).
...
- Nur macOS: `Voracode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
#### Globale Konfiguration prÃ¼fen

Oeffne deine globale Konfigurationsdatei und suche nach dem `plugin`-Schluessel.

- **macOS/Linux**: `~/.config/voracode/voracode.jsonc` (oder `~/.config/voracode/voracode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/voracode/voracode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\voracode\voracode.jsonc`
...
#### Plugin-Verzeichnisse prÃ¼fen

Voracode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/voracode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\voracode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Voracode ihn neu aufbauen kann.

1. Beende Voracode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/voracode`
- **Linux**: loesche `~/.cache/voracode` (oder fuehre aus: `rm -rf ~/.cache/voracode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\voracode`

3. Starte Voracode Desktop neu.

---

### Server-Verbindungsprobleme beheben

Voracode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
#### Desktop-Standard-Server-URL lÃ¶schen
...
Wenn deine `voracode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prÃ¼fen

Wenn du `VORACODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `VORACODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Voracode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Voracode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Voracode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Voracode in den OS-Einstellungen aktiviert sind, und
- das App-Fenster nicht fokussiert ist.

---

### Desktop-App-Speicher zurÃ¼cksetzen (letzter Ausweg)
...
1. Beende Voracode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Voracode Desktop):

- `voracode.settings.dat` (desktop default server URL)
- `voracode.global.dat` und `voracode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Voracode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `voracode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\voracode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. voracode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\voracode`

2. Starte voracode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt voracode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
voracode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
