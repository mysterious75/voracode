...
Voracode bringt eingebaute Tools mit und laesst sich ueber [Custom Tools](/docs/custom-tools) oder [MCP-Server](/docs/mcp-servers) erweitern.
...
Hier sind alle in Voracode verfuegbaren eingebauten Tools.
...
:::note
Dieses Tool ist nur verfuegbar, wenn `VORACODE_EXPERIMENTAL_LSP_TOOL=true` (oder `VORACODE_EXPERIMENTAL=true`) gesetzt ist.
:::
...
:::note
Dieses Tool ist nur verfuegbar, wenn der Voracode-Provider genutzt wird oder die Umgebungsvariable `VORACODE_ENABLE_EXA` auf einen 'truthy' Wert (z. B. `true` oder `1`) gesetzt ist.

Zum Aktivieren beim Start von Voracode:
...
Kein API-Key erforderlich â€” das Tool verbindet sich direkt mit dem gehosteten MCP-Service von Exa AI ohne Authentifizierung.
