...
With Voracode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Voracode uses our own `voracode` theme.
...
Voracode comes with several built-in themes.
...
- Want Voracode to match their terminal's appearance
...
Voracode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/voracode/themes/*.json` or `$XDG_CONFIG_HOME/voracode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.voracode/themes/*.json`
4. **Current working directory** - Defined in `./.voracode/themes/*.json`
