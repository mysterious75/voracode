...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("voracode-find")
  api.delete("voracode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("voracode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("voracode-find", new Highlight(...rest))
    return true
  }
