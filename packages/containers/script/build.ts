...
const reg = process.env.REGISTRY ?? "ghcr.io/mysterious75"
...
const setup = async () => {
  ...
  if (list.includes("voracode")) {
    await $`docker buildx use voracode`
    return
  }
  await $`docker buildx create --name voracode --use`
}
