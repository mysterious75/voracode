...
import { SessionStatusEvent } from "@voracode-ai/schema/session-status-event"
import { SessionV1 } from "@voracode-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
import { mockVoracodeServer } from "../../utils/mock-server"
import { installSseTransport } from "../../utils/sse-transport"
import { expectSessionTitle } from "../../utils/waits"

export const directory = "C:/Voracode/TimelineStability"
...
export const model = { providerID: "voracode", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("voracode.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["voracode"],
    default: { providerID: "voracode", modelID: "claude-opus-4-6" },
  }
}
