...
const directory = "C:/Voracode/FileBrowserSidebar"
...
async function setup(page: Page) {
  await mockVoracodeServer(page, {
    ...
    provider: {
      ...
      connected: ["voracode"],
      default: { providerID: "voracode", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
  ...
  await page.addInitScript(
    ({ directory, server, sessionID }) => {
      ...
      localStorage.setItem(
        "voracode.global.dat:layout",
        JSON.stringify({ review: { diffStyle: "split", panelOpened: true } }),
      )
      localStorage.setItem(
        "voracode.global.dat:review-panel-v2",
        JSON.stringify({ sidebarOpened: true, sidebarWidth: 240, expandMode: "collapse" }),
      )
      localStorage.setItem(
        "voracode.window.browser.dat:tabs",
