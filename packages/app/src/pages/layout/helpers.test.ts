...
import { type Session } from "@voracode-ai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("voracode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("voracode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("voracode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("voracode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("voracode://open-project")).toBeUndefined()
    expect(parseDeepLink("voracode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "voracode://open-project?directory=/a",
      "voracode://other?directory=/b",
      "voracode://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("voracode://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("voracode://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("voracode://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("voracode://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "voracode://new-session?directory=/a",
      "voracode://open-project?directory=/b",
      "voracode://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __VORACODE__: {
        deepLinks: ["voracode://open-project?directory=/a"],
      },
    } as unknown as Window & { __VORACODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["voracode://open-project?directory=/a"])
