...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-voracode" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslvoracodeAction(check?: WslvoracodeCheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install Voracode"
  if (check.matchesDesktop === false) return "Update Voracode"
}
...
export function addServerViewModel(input: {
  ...
  const voracodeCheck = selectedDistro ? (state?.voracodeChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.voracodeChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updatevoracode" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.voracodeMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installvoracode" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  voracodeCheck: WslvoracodeCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingvoracode =
    input.state?.job?.kind === "install-voracode" && input.state.job.distro === input.selectedDistro
  if (!ready || probingvoracode) {
    return {
      variant: "contrast",
  ...
  if (!addServervoracodeReady(input.voracodeCheck)) {
    const update = !!input.voracodeCheck?.resolvedPath && input.voracodeCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingvoracode
        ? { key: "wsl.onboarding.updatingvoracode" }
        : update
          ? { key: "wsl.onboarding.updatevoracode" }
          : { key: "wsl.onboarding.installvoracode" },
      disabled: !!input.state?.job || input.adding,
      action: "install-voracode",
      loading: installingvoracode,
...
function addServervoracodeReady(check: WslvoracodeCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.voracodeChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.voracodeChecks[item.name]) return [`voracode:${item.name}`]
    return []
  })
