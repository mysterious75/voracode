...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when Voracode needs attention", () => {
    expect(wslvoracodeAction(undefined)).toBeUndefined()
    expect(
      wslvoracodeAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("Install Voracode")
    expect(
      wslvoracodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/voracode",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("Update Voracode")
    expect(
      wslvoracodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/voracode",
  ...
  test("does not report ready when Voracode is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        voracodeChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.voracode/bin/voracode",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installvoracode" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-voracode")
  })
