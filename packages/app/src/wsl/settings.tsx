...
import { Tag } from "@voracode-ai/ui/v2/badge-v2"
...
import { wslvoracodeAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.voracodeChecks[item.config.distro]
          const voracodeAction = () => wslvoracodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-voracode" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={voracodeAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installvoracode(item.config.distro))}
                    >
