  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://voracode.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://voracode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://voracode.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://voracode.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://voracode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://voracode.example.com"),
      ),
    ).toEqual({
