import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@voracode-ai/sdk/v2/client"
...
import { List, type ListRef } from "@voracode-ai/ui/list"
import { ProviderIcon } from "@voracode-ai/ui/provider-icon"
import { Spinner } from "@voracode-ai/ui/spinner"
import { Tag } from "@voracode-ai/ui/tag"
...
function ProviderPicker(props: { directory?: Accessor<string | undefined>; onSelect: (provider: string) => void }) {
  ...
  const note = (id: string) => {
    ...
    if (id === "voracode-go") return language.t("dialog.provider.voracodeGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "voracode"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.voracode.tagline")}</div>
          </Show>
  ...
  function ApiAuthView() {
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "voracode"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.voracodeZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.voracodeZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.voracodeZen.visit.prefix")}
                <Link href="https://voracode.ai/zen" tabIndex={-1}>
                  {language.t("provider.connect.voracodeZen.visit.link")}
                </Link>
                {language.t("provider.connect.voracodeZen.visit.suffix")}
              </div>
            </div>
          </Match>
          <Match when={true}>
