...
import { getvoracodeModeStack, VORACODE_BASE_MODE, VoracodeKeymapProvider, registervoracodeKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registervoracodeKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when voracode mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getvoracodeModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getvoracodeModeStack(keymap).push("autocomplete")
