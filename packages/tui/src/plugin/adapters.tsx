import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@voracode-ai/plugin/tui"
...
import type { usevoracodeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof usevoracodeKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getvoracodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getvoracodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
