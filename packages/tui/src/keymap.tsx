...
export const VORACODE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const VORACODE_MODE_KEY = "voracode.mode"

export const VoracodeKeymapProvider = KeymapProvider
export const usevoracodeKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type voracodeModeStack = ReturnType<typeof createvoracodeModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, voracodeModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createvoracodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(VORACODE_MODE_KEY, VORACODE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(VORACODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(VORACODE_MODE_KEY, stack.at(-1)?.mode ?? VORACODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? VORACODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(VORACODE_MODE_KEY, undefined)
...
export function usevoracodeModeStack() {
  return getvoracodeModeStack(usevoracodeKeymap())
}

export function getvoracodeModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Voracode mode stack is not registered for this keymap")
  return value
}
...
export function registervoracodeKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createvoracodeModeStack(keymap)
...
export function useCommandSlashes(): Accessor<readonly CommandSlashEntry[]> {
  const keymap = usevoracodeKeymap()
