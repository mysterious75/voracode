...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve voracode route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "voracode")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "voracode")).toBe("openai")
    expect(statProvider("big-pickle", "", "voracode")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "voracode"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep voracode as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "voracode"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "voracode"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep voracode or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "voracode"), country: "US" })).toMatchObject([
