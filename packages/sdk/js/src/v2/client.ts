...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-voracode-directory", "directory"],
    ["x-voracode-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-voracode-workspace")
  return next
}

export function createvoracodeClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  if (!config?.fetch) {
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-voracode-workspace": config.experimental_workspaceID,
    }
  }
  ...
  client.interceptors.response.use((response) => {
    ...
    if (contentType === "text/html")
      throw new Error("Request is not supported by this version of Voracode Server (Server responded with text/html)")
