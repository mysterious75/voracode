...
import { and, Database, eq, isNull, lt, or, sql } from "@voracode-ai/console-core/drizzle/index.js"
import { KeyTable } from "@voracode-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@voracode-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@voracode-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@voracode-ai/console-core/util/date.js"
import { Identifier } from "@voracode-ai/console-core/identifier.js"
import { Billing } from "@voracode-ai/console-core/billing.js"
import { Actor } from "@voracode-ai/console-core/actor.js"
import { WorkspaceTable } from "@voracode-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@voracode-ai/console-core/model.js"
import { Subscription } from "@voracode-ai/console-core/subscription.js"
import { BlackData } from "@voracode-ai/console-core/black.js"
import { UserTable } from "@voracode-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@voracode-ai/console-core/schema/model.sql.js"
import { ProviderTable } from "@voracode-ai/console-core/schema/provider.sql.js"
...
import { LiteData } from "@voracode-ai/console-core/lite.js"
import { Resource } from "@voracode-ai/console-resource"
...
import { Workspace } from "@voracode-ai/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-voracode-session") ?? ""
    const requestId = input.request.headers.get("x-voracode-request") ?? ""
    const ocClient = input.request.headers.get("x-voracode-client") ?? ""
    const projectId = input.request.headers.get("x-voracode-project") ?? ""
      ...
      if (!allowedRegions?.includes("unavailable"))
        throw new RegionError(
          t("zen.api.error.regionNotAllowed", {
            consoleGoUrl: `https://voracode.ai/workspace/${authInfo.workspaceID}/go`,
          }),
        )
    }
    */
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWithRetryableStatus(
        reqUrl,
        {
          ...
          headers: (() => {
            ...
            headers.delete("x-voracode-request")
            headers.delete("x-voracode-session")
            headers.delete("x-voracode-project")
            headers.delete("x-voracode-client")
            return headers
          })(),
          body: reqBody,
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-voracode-endpoint-id")
        const resEndpointModelId = res.headers.get("x-voracode-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://voracode.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://voracode.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://voracode.ai/workspace/${authInfo.workspaceID}/members`
