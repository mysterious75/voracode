...
const TUI_SCHEMA_URL = "https://voracode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from voracode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const voracode = await voracodeFiles(input)
  for (const file of voracode) {
...
async function voracodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "voracode"),
    ...(await Filesystem.findUp(["voracode.json", "voracode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "voracode"))
  }
  if (Flag.VORACODE_CONFIG) files.push(Flag.VORACODE_CONFIG)
