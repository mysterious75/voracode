...
import { isRecord } from "@voracode-ai/tui/util/record"
...
import { TuiKeybind } from "@voracode-ai/tui/config/keybind"
...
import { TuiConfig } from "@voracode-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@voracode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old voracode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.voracode`
  // folders between cwd and home, and VORACODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.VORACODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit VORACODE_TUI_CONFIG override, if set.
  if (Flag.VORACODE_TUI_CONFIG) {
    const configFile = Flag.VORACODE_TUI_CONFIG
  ...
  // 4. `.voracode` directories (and VORACODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".voracode") || dir === Flag.VORACODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".voracode") && dir !== Flag.VORACODE_CONFIG_DIR) continue
