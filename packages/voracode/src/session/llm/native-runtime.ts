...
import type { LLMClientShape } from "@voracode-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("voracode"))
    return { type: "unsupported", reason: "provider is not openai, voracode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @voracode-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains voracode-owned. The native runtime only adapts
      // the @voracode-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
