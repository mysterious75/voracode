import type { PermissionV1 } from "@voracode-ai/core/v1/permission"
import { FSUtil } from "@voracode-ai/core/fs-util"
// CLI entry point for `voracode run` and `voracode --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`voracode --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`voracode --mini --attach`): connects to a running
//      voracode server and runs interactive mode against it.
//
...
import { createvoracodeClient, type voracodeClient, type ToolPart } from "@voracode-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<voracodeClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run voracode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running voracode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to VORACODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to VORACODE_SERVER_USERNAME or 'voracode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: voracodeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: voracodeClient, sessionID: string) {
      ...
      async function current(sdk: voracodeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: voracodeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: voracodeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: voracodeClient) {
        ...
        async function loop(client: voracodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createvoracodeClient({
        baseUrl: "http://voracode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
