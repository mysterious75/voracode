// Shared type vocabulary for the direct interactive mode (`voracode --mini`).
//
...
import type { voracodeClient, PermissionRequest, QuestionRequest, ToolPart } from "@voracode-ai/sdk/v2"
import type { TuiConfig } from "@voracode-ai/tui/config"
...
type PromptModel = Parameters<voracodeClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<voracodeClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<voracodeClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<voracodeClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<voracodeClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<voracodeClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<voracodeClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<voracodeClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<voracodeClient["question"]["reject"]>[0]
