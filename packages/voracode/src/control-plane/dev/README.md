This is a plugin to simulate a remote environment locally. Add this to `.voracode/voracode.jsonc`:

```json
  "plugin": ["../packages/voracode/src/control-plane/dev/debug-workspace-plugin.ts"],
```

In a separate terminal, run a separate Voracode server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/voracode/script/run-workspace-server
```

With the plugin install, you can now run Voracode and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
