...
import { IdeEvent } from "@voracode-ai/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["VORACODE_CALLER"] === "vscode" || process.env["VORACODE_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.voracode"], {
    nothrow: true,
  })
