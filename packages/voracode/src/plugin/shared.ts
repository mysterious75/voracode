...
export const DEPRECATED_PLUGIN_PACKAGES = ["voracode-openai-codex-auth", "voracode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, voracodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(voracodeVersion) || semver.major(voracodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.voracode
  if (typeof range !== "string") return
  if (!semver.satisfies(voracodeVersion, range)) {
    throw new Error(`Plugin requires voracode ${range} but running ${voracodeVersion}`)
  }
}
