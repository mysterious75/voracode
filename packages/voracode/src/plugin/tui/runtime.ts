...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@voracode-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@voracode-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@voracode-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@voracode-ai/tui/context/theme"
import { Global } from "@voracode-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@voracode-ai/core/util/flock"
import { Flag } from "@voracode-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@voracode-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@voracode-ai/core/v1/config/plugin"
import { createCommandShim } from "@voracode-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@voracode-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".voracode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".voracode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".voracode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.VORACODE_PURE ? [] : pluginOrigins
    if (Flag.VORACODE_PURE && pluginOrigins.length) {
    }
