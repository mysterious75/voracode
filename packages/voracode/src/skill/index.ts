...
import { FrontmatterError } from "@voracode-ai/core/v1/config/error"
...
const VORACODE_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
const SKILL_PATTERN = "**/SKILL.md"

// Built-in skill that ships with voracode. The model's intuition for what an
// voracode.json should look like is often wrong, and voracode hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch voracode's own config files gives it the
// actual schemas instead of guesses.
const CUSTOMIZE_VORACODE_SKILL_NAME = "customize-voracode"
const CUSTOMIZE_VORACODE_SKILL_DESCRIPTION =
  "Use ONLY when the user is editing or creating voracode's own configuration: voracode.json, voracode.jsonc, files under .voracode/, or files under ~/.config/voracode/. Also use when creating or fixing voracode agents, subagents, skills, plugins, MCP servers, or permission rules. Do not use for the user's own application code, or for any project that is not configuring voracode itself."
const CUSTOMIZE_VORACODE_SKILL_BODY = SkillPlugin.CustomizevoracodeContent
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, VORACODE_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@voracode/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_VORACODE_SKILL_NAME] = {
          name: CUSTOMIZE_VORACODE_SKILL_NAME,
          description: CUSTOMIZE_VORACODE_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_VORACODE_SKILL_BODY,
        }
