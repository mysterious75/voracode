...
import { httpClient, path } from "@voracode-ai/core/effect/app-node-platform"
...
export class Service extends Context.Service<Service, Interface>()("@voracode/SkillDiscovery") {}
      ...
      const dirs = yield* Effect.forEach(
        ...
        (skill) =>
          Effect.gen(function* () {
            ...
            const versionFile = path.join(root, ".voracode-version")
            ...
            if (version === undefined || current === version) {
              ...
              yield* Effect.gen(function* () {
                ...
                yield* fs.writeFileString(path.join(staging, ".voracode-version"), version)
...
const skillKeywords = [
  "always", "every time", "whenever", "create a", "set up", "configure",
  "deploy", "migrate", "convert", "optimize",
]

const stepPatterns = [
  /^\d+\.\s/m,
  /^[-*]\s.*:/m,
  /^(first|next|then|finally|step)\b/im,
]

export async function discoverSkillFromInteraction(
  description: string,
  output: string,
): Promise<{ suggested: boolean; name?: string; prompt?: string }> {
  const lower = description.toLowerCase()
  const hasKeyword = skillKeywords.some((kw) => lower.includes(kw))
  const hasSteps = stepPatterns.some((p) => p.test(output))
  const hasActions = /(run|execute|install|build|test|deploy|create|edit|write)\s/i.test(output)

  if (!hasKeyword && !(hasSteps && hasActions)) {
