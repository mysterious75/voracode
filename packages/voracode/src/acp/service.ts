...
import type { AssistantMessage, Message, voracodeClient, SessionMessageResponse } from "@voracode-ai/sdk/v2"
...
export const AuthMethodID = "voracode-login"
...
export class Service extends Context.Service<Service, Interface>()("@voracode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `voracode auth login` in the terminal",
      name: "Login with voracode",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: voracodeClient) {
...
function makeUsageService(sdk: voracodeClient) {
...
async function loadDirectorySnapshot(sdk: voracodeClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, voracode provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const voracodeProvider = providers[ProviderV2.ID.make("voracode")]
  const voracodeModel = voracodeProvider ? Provider.sort(Object.values(voracodeProvider.models))[0] : undefined
  if (voracodeProvider && voracodeModel) return { providerID: voracodeProvider.id, modelID: voracodeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Voracode service failure", service })
}
