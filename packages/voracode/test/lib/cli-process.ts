// Subprocess test harness for the voracode CLI. Spawns the real binary against
...
//
// Configuration flows through voracode's built-in test affordances:
//   - VORACODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - VORACODE_TEST_HOME           : pins os.homedir() â†’ tmpdir
//   - VORACODE_DISABLE_PROJECT_CONFIG : skip walking up for voracode.json
//   - VORACODE_PURE                : skip external plugin discovery + install
//   - VORACODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `voracode.run` is fully wired. The shape supports adding more
// builders (`voracode.serve(opts)`, `voracode.acp(opts)`, `voracode.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape â€” see the TODO at the bottom of voracodeCli.
...
import { AppProcess } from "@voracode-ai/core/process"
...
const voracodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(voracodeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    VORACODE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    VORACODE_CONFIG_CONTENT: configJson,
    VORACODE_DISABLE_PROJECT_CONFIG: "1",
    VORACODE_PURE: "1",
    VORACODE_DISABLE_AUTOUPDATE: "1",
    VORACODE_DISABLE_AUTOCOMPACT: "1",
    VORACODE_DISABLE_MODELS_FETCH: "1",
    VORACODE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `voracode run`. New flags should
...
// `voracode serve` is a long-lived process â€” it never exits on its own.
...
// `voracode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type voracodeCli = {
  ...
  // Spawn `voracode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `voracode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("voracode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          VORACODE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("voracode.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("voracode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "voracode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `voracode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("voracode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const voracode: voracodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, voracode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `voracode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "voracode") {
...
// every call site. Use this for any test that needs the voracode CLI fixture.
//
...
// resources (e.g. `voracode.serve`) without an extra `Effect.scoped` wrapper â€”
