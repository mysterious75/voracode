...
import { makeGlobalNode } from "@voracode-ai/core/effect/app-node"
...
import { InstallationChannel } from "@voracode-ai/core/installation/version"
  ...
  describe("latest", () => {
    ...
    testEffect(
      ...
      Effect.gen(function* () {
        ...
        expect(npmCalls).toContain(`https://registry.npmjs.org/voracode-ai/${InstallationChannel}`)
      }),
    )
    ...
    testEffect(
      ...
      Effect.gen(function* () {
        ...
        expect(bunCalls).toContain(`https://registry.npmjs.org/voracode-ai/${InstallationChannel}`)
      }),
    )
    ...
    testEffect(
      ...
      Effect.gen(function* () {
        ...
        expect(pnpmCalls).toContain(`https://registry.npmjs.org/voracode-ai/${InstallationChannel}`)
      }),
    )
    ...
    testEffect(
      testLayer(
        ...
        (cmd, args) => {
          ...
          if (cmd === "brew" && args.includes("--formula") && args.includes("mysterious75/tap/voracode")) return ""
          if (cmd === "brew" && args.includes("--formula") && args.includes("voracode")) return "voracode"
          return ""
        },
      ),
    ...
    testEffect(
      testLayer(
        ...
        (cmd, args) => {
          if (cmd === "brew" && args.includes("mysterious75/tap/voracode") && args.includes("--formula")) return "voracode"
