...
import type { Event, voracodeClient } from "@voracode-ai/sdk/v2"
...
type PermissionReplyParams = Parameters<voracodeClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "voracode-acp-permission-"))
