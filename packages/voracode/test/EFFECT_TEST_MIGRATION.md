...
it.instance("instance-local behavior", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("voracode-test-")
  }),
)
...
- `it.effect(...)` â€” pure Effect behavior with `TestClock` and
  `TestConsole`.
- `it.instance(...)` â€” service behavior that needs one scoped voracode
  instance.
...
9. Run the focused test file and `bun typecheck` from `packages/voracode`.
...
```bash
git grep -n "Effect.runPromise\|ManagedRuntime\|Promise.withResolvers\|Bun.sleep\|withTestInstance" -- packages/voracode/test
```
