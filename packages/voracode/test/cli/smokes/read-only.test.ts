...
describe("voracode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // VORACODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ voracode }) =>
      Effect.gen(function* () {
        const r = yield* voracode.spawn(["mcp", "list"])
        voracode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ voracode }) =>
      Effect.gen(function* () {
        const r = yield* voracode.spawn(["providers", "list"])
        voracode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ voracode }) =>
      Effect.gen(function* () {
        const r = yield* voracode.spawn(["models"])
        voracode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ voracode }) =>
      Effect.gen(function* () {
        const r = yield* voracode.spawn(["agent", "list"])
        voracode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh VORACODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ voracode }) =>
      Effect.gen(function* () {
        const r = yield* voracode.spawn(["session", "list"])
        voracode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ voracode }) =>
      Effect.gen(function* () {
        const r = yield* voracode.spawn(["stats"])
        voracode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under VORACODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form â€” both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ voracode }) =>
      Effect.gen(function* () {
        const r = yield* voracode.spawn(["db", "path"])
        voracode.expectExit(r, 0, "db path")
