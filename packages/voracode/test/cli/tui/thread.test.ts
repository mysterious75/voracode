...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ voracode }) =>
    Effect.gen(function* () {
      const result = yield* voracode.spawn(["--replay-limit", "10"])

      voracode.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ voracode }) =>
    Effect.gen(function* () {
      const result = yield* voracode.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ voracode }) =>
    Effect.gen(function* () {
      const result = yield* voracode.spawn(["--mini", "--port", "4096"])
