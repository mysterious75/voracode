...
describe("voracode acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ voracode }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ voracode }))
        ...
        expect(initialized.agentInfo?.name).toBe("Voracode")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ voracode }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ voracode })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("voracode-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "voracode-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.VORACODE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ voracode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("voracode-login")
