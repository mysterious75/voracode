...
export function createAcpClient(input: Pick<CliFixture, "voracode">, env?: Record<string, string>) {
  return Effect.gen(function* () {
    return createJsonRpcAcpClient(yield* input.voracode.acp(env ? { env } : undefined))
  })
}
  ...
  return Effect.gen(function* () {
    return expectOk(
      yield* acp.request<InitializeResponse>("initialize", {
        ...
        clientInfo: { name: "voracode-local-acp", version: "0.1.0" },
      }),
    )
  })
}
