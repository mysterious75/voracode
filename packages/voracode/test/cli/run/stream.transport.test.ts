...
import { voracodeClient, type GlobalEvent } from "@voracode-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<voracodeClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<voracodeClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<voracodeClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<voracodeClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<voracodeClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://voracode.test"),
...
function sdk(
  input: {
    ...
    subscribe?: voracodeClient["event"]["subscribe"]
    globalEvent?: voracodeClient["global"]["event"]
    promptAsync?: voracodeClient["session"]["promptAsync"]
    status?: voracodeClient["session"]["status"]
    messages?: voracodeClient["session"]["messages"]
    children?: voracodeClient["session"]["children"]
    permissions?: voracodeClient["permission"]["list"]
    questions?: voracodeClient["question"]["list"]
  } = {},
) {
  const client = new voracodeClient()

  const subscribe: voracodeClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: voracodeClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: voracodeClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: voracodeClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: voracodeClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: voracodeClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: voracodeClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: voracodeClient["question"]["list"] = input.questions ?? (() => ok([]))
