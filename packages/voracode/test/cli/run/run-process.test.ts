// Subprocess integration tests for `voracode run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness â€” each test uses
// `voracode.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `VORACODE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("voracode run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, voracode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* voracode.run("say hi")
        voracode.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, voracode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* voracode.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, voracode }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* voracode.run("think", { extraArgs: ["--thinking"] })
        voracode.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* voracode.run("think again")
        voracode.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, voracode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* voracode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, voracode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* voracode.run("say hi", { format: "json" })
        voracode.expectExit(result, 0)

        const events = voracode.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ voracode }) =>
      Effect.gen(function* () {
        const result = yield* voracode.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, voracode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* voracode.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, voracode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* voracode.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, voracode }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* voracode.run("request permission", { permission: { bash: "ask" } })
        voracode.expectExit(denied, 0)
        ...
        const allowed = yield* voracode.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        voracode.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* voracode.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        voracode.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, voracode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* voracode.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, voracode }) =>
      Effect.gen(function* () {
        const result = yield* voracode.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, voracode }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* voracode.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
