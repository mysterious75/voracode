...
import { SessionV1 } from "@voracode-ai/core/v1/session"
import { ModelsDev } from "@voracode-ai/core/models-dev"
import { HttpRecorder } from "@voracode-ai/http-recorder"
import { HttpRecorderInternal } from "@voracode-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@voracode-ai/llm"
import { RequestExecutor } from "@voracode-ai/llm/route"
...
const zenURL = (connection: string) => `https://console.voracode.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.VORACODE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["voracode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("VORACODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("VORACODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["voracode", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "voracode-proxy",
    name: "Voracode proxy",
    providerID: ProviderV2.ID.voracode,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["voracode", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.VORACODE_RECORD_CONSOLE_TOKEN && process.env.VORACODE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.voracode,
        name: "Voracode Zen",
        env: ["VORACODE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.VORACODE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.VORACODE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.VORACODE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["voracode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("VORACODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("VORACODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("VORACODE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set VORACODE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the voracode LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "voracode.json"),
      JSON.stringify({ $schema: "https://voracode.ai/config.json", ...scenario.config(model) }),
    ),
  )
