...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-voracode-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-voracode-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://voracode.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-voracode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-voracode-workspace")
