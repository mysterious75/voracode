...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "voracode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "voracode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "voracode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "voracode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["voracode.json", "voracode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "voracode.json"),
        path.join(project, "voracode.jsonc"),
      ])
    })
  })
