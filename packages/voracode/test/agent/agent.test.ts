...
it.instance("plan agent denies edits except .voracode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".voracode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".voracode", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.VORACODE_TEST_HOME
      process.env.VORACODE_TEST_HOME = test.directory
