...
import { EventSequenceTable } from "@voracode-ai/core/event/sql"
...
import { WorkspaceTable } from "@voracode-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  VORACODE_AUTH_CONTENT: process.env.VORACODE_AUTH_CONTENT,
  VORACODE_EXPERIMENTAL_WORKSPACES: process.env.VORACODE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.VORACODE_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@voracode.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.VORACODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=voracode-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.VORACODE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.VORACODE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.VORACODE_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=voracode-test")
