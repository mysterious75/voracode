...
Trim `packages/voracode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/voracode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/voracode`.
- `[x]` `bun typecheck` from `packages/voracode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/voracode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/voracode`.
- `[x]` `bun dev generate > /tmp/voracode-openapi.json` from `packages/voracode`.
...
- Edit `packages/voracode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/voracode-openapi.json` from `packages/voracode`.
...
- Check source schemas in files like `packages/voracode/src/session/schema.ts`, `packages/voracode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/voracode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/voracode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
