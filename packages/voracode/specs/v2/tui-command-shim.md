...
Delete `packages/voracode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/voracode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/voracode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/voracode`
- TUI plugin loader tests in `packages/voracode` if runtime plugin API wiring changed
