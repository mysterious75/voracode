...
Practical reference for a future `packages/server` split after the voracode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/voracode`.
...
Until enough shared service code lives outside `packages/voracode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/voracode`

It should not import `packages/voracode` services while `packages/voracode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on voracode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/voracode` implementation details.
