...
- Author package entrypoint is `@voracode-ai/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/voracode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@voracode-ai/plugin/tui`.
- `@voracode-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@voracode-ai/plugin/tui"
...
- The value is a semver range checked against the running Voracode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.voracode` is absent, no check is performed (backward compatible).
...
- `voracode plugin <module>` and TUI install both run install â†’ manifest read â†’ config patch.
- Alias: `voracode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.voracode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.voracode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.voracode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.voracode` dir or `VORACODE_CONFIG_DIR`, Voracode installs `@voracode-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@voracode-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Voracode's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Voracode registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@voracode-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"voracode"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `voracode.default`.
...
- Local plugins persist installed themes under the local `.voracode/themes` area near the plugin config source.
...
- `--pure` / `VORACODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.voracode/plugins/tui-smoke.tsx`
- Local vim plugin: `.voracode/plugins/tui-vim.tsx`
- Local smoke config: `.voracode/tui.json`
- Local smoke theme: `.voracode/plugins/smoke-theme.json`
