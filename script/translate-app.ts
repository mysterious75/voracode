...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "voracode/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".voracode/glossary/zh-cn.md"
  if (locale === "zht") return ".voracode/glossary/zh-tw.md"
  return `.voracode/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Voracode did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("Voracode returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Voracode session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel Voracode runs for 'all' (default: 4)
      --model <provider/id>  Voracode model (default: voracode/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Voracode
...
`)
  ...
  if (failed.length) console.error(`\nVoracode failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.VORACODE_DISABLE_PROJECT_CONFIG = "1"
  env.VORACODE_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["voracode", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.VORACODE_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["voracode", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.VORACODE_CONFIG
  delete env.VORACODE_CONFIG_DIR
  delete env.VORACODE_CONFIG_CONTENT
  delete env.VORACODE_PERMISSION
  delete env.VORACODE_AUTO_SHARE
  return env
}
